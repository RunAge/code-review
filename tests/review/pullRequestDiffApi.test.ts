...
describe("fetchPullRequestDiff", () => {
  it("requests PR diff with GitHub diff accept header", async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response("diff --git a/a b/a", { status: 200 }));
  ...
  it("throws when GitHub returns non-OK response", async () => {
    ...
    await expect(
      fetchPullRequestDiff({ owner: "acme", repo: "tool", pullNumber: 12 }, fetchMock as unknown as typeof fetch)
